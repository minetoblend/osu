...
namespace osu.Game.Tests.Visual.Matchmaking
{
    public partial class TestScenePickScreen : MultiplayerTestScene
    {
        ...
        {
            var selectedItems = new List<PlaylistItem>();
            ...
            {
                long[] candidateItems = selectedItems.Select(it => it.ID).Distinct().ToArray();
                long finalItem = candidateItems[Random.Shared.Next(candidateItems.Length)];

                screen.PresentFinalBeatmap(candidateItems, finalItem);
            });
        }
    }
}
