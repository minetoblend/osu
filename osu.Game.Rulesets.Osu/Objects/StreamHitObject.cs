...
{
    public class StreamHitObject : HitObjectWithPath
    {
        public float DurationPerCircle;

        protected override void updateNestedPositions()
        {
            base.updateNestedPositions();
            ClearNested();
        }
        ...
        {
            int numHitObjects = (int)(Duration / DurationPerCircle + 1.5);
            ...
            for (int i = 0; i < numHitObjects; i++)
            {
                ...
                var hitObject = (HitCircle)NestedHitObjects[i];
