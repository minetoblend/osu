...
namespace osu.Game.Rulesets.Osu.Edit.Blueprints.Streams
{
    public class StreamSelectionBlueprint : OsuSelectionBlueprint<Stream>
    {
        ...
        [CanBeNull]
        protected Container<HitCirclePiece> CircleContainer { get; private set; }
        ...
        public override Quad SelectionQuad
        {
            get
            {
                ...
                foreach (var p in DrawableObject.NestedHitObjects)
                {
                    minPosition = Vector2.ComponentMin(minPosition,
                        ToScreenSpace(p.Position - new Vector2((float)((OsuHitObject)p.HitObject).Radius))
                    );
                    maxPosition = Vector2.ComponentMax(maxPosition,
                        ToScreenSpace(p.Position + new Vector2((float)((OsuHitObject)p.HitObject).Radius))
                    );
                }
        ...
        {
            AddRangeInternal(new CompositeDrawable[]
            {
                ControlPointVisualiser = new PathControlPointVisualiser(HitObject, true)
                {
                    RemoveControlPointsRequested = removeControlPoints
                },
                CircleContainer = new Container<HitCirclePiece>
                {
        ...
        private void removeControlPoints(List<PathControlPoint> toRemove)
        {
            ...
            foreach (var c in toRemove)
            {
                ...
                if (c == controlPoints[0] && controlPoints.Count > 1 && controlPoints[1].Type == null)
                    controlPoints[1].Type = controlPoints[0].Type.Value;
        ...
        private void updatePath()
        {
            ...
            float? snapDistance = composer?.GetBeatSnapDistanceAt(HitObject);

            if (snapDistance != null)
                HitObject.DurationPerCircle = (float)snapDistance * 2;
            ...
            if (CircleContainer != null)
            {
                ...
                {
                    if (HitObject.NestedHitObjects[i] is StreamHitCircle circle)
                    {
